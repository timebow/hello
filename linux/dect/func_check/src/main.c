...
#define    AP03_HM13_ARC_MASK                             ( u8 )0x70
#define    AP03_HM13_ACCESS_CLASS_A_MASK                  ( u8 )0x00
#define    AP03_HM13_ACCESS_CLASS_B_MASK                  ( u8 )0x10

u8 p_hm13_IsSameRfpiWithoutRpn(u8 *pu8_Rfpi1, u8 *pu8_Rfpi2)
{
    u8 u8_RPNMask = 0;

    if(!pu8_Rfpi1 || !pu8_Rfpi2)
        return 0;

    if ((pu8_Rfpi1[0] & AP03_HM13_ARC_MASK ) == AP03_HM13_ACCESS_CLASS_A_MASK)
        u8_RPNMask = 0x07;
    else
        u8_RPNMask = 0xFF;
    
    printf("Mask: 0x%x\n", u8_RPNMask);
    printf("RFPI1: %02x%02x%02x%02x%02x\n", pu8_Rfpi1[0], pu8_Rfpi1[1], pu8_Rfpi1[2], pu8_Rfpi1[3], pu8_Rfpi1[4]);
    printf("RFPI2: %02x%02x%02x%02x%02x\n", pu8_Rfpi2[0], pu8_Rfpi2[1], pu8_Rfpi2[2], pu8_Rfpi2[3], pu8_Rfpi2[4]);

    if( ( pu8_Rfpi1[0] != pu8_Rfpi2[0] )
      ||( pu8_Rfpi1[1] != pu8_Rfpi2[1] )
      ||( pu8_Rfpi1[2] != pu8_Rfpi2[2] )
      ||( pu8_Rfpi1[3] != pu8_Rfpi2[3] )
      ||((pu8_Rfpi1[4] & ~u8_RPNMask) != (pu8_Rfpi2[4] & ~u8_RPNMask)))
    {
        return 0;
    }
...
{
    unsigned char RFPI1[5];
    unsigned char RFPI2[5];
    unsigned char ret = 0;
    char c;
    ...
    while(1)
    {
        printf("Please input RFPI1 (5 Bytes):\n");
        printf("> ");
        if(5 != scanf("%02x%*c%02x%*c%02x%*c%02x%*c%02x", &RFPI1[0], &RFPI1[1], &RFPI1[2], &RFPI1[3], &RFPI1[4]))
        {
            printf("INPUT ERROR!\n");
            while((c=getchar())!='\n'&&c!=EOF);
            continue;
        }
        printf("Please input RFPI2 (5 Bytes):\n");
        printf("> ");
        if(5 != scanf("%02x%*c%02x%*c%02x%*c%02x%*c%02x", &RFPI2[0], &RFPI2[1], &RFPI2[2], &RFPI2[3], &RFPI2[4]))
        {
            printf("INPUT ERROR!\n");
            while((c=getchar())!='\n'&&c!=EOF);
            continue;
        ...
        /*解析AField*/
        ret = p_hm13_IsSameRfpiWithoutRpn(RFPI1, RFPI2);
        printf("check ret = %d\n", ret);
    }
}
